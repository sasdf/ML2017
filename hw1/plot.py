...
y = []
z = []
for l in range(len(train)/100, len(train), len(train)/100):
    ker = adagrad(kernel, train[:l], best_lr, best_lam, 2000)
    val_loss = rmse(ker, validate)
    tra_loss = rmse(ker, train[:l])
    x.append(l)
    y.append(val_loss)
    z.append(tra_loss)
    print "rate: %f, lam: %.20f, train : %f, verify: %f" %(best_lr, best_lam, tra_loss, val_loss)
...
plt.legend(handles=[
plt.plot(x, y, label="validation loss")[0],
plt.plot(x, z, label="train loss")[0]
])
plt.show()
