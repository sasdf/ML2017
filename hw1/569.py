...
def extract(df):
    ...
    df = df[['PM2.5', 'Ans']]
    x = df['PM2.5']
    df['PM2.5l'] = (x+100000).map(math.log).values.flatten()
...
validate = dataset[:len(dataset)/15]
...
for elr in range(6):
    lr = pow(10, -elr)
    for elam in range(0, 9):
        lam = pow(10, -elam)
        ker = adagrad(kernel, train, lr, lam, 4000)
        ...
        if val_loss < best_loss:
            ...
            best_lr = lr
            best_lam = lam
        print "rate: %f, lam: %f, train : %f, verify: %f" %(lr, lam, rmse(ker, train), val_loss)

print "Recalculate best kernel"
best_ker = adagrad(kernel, dataset, best_lr, best_lam, 12000)
print "rate: %f, lam: %f, train : %f" %(best_lr, best_lam, rmse(best_ker, dataset))
print best_ker
