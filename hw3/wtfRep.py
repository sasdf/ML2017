...
for i in range(4):
    ...
    yo = BatchNormalization()(yo)
    if i != 0 : 
        yo = MaxPooling2D(2,2)(yo)
    yo = Dropout(0.1)(yo)

yo = Convolution2D(200,(3,3),
    kernel_initializer = 'glorot_normal',
    kernel_regularizer=l2(0.0000001),
)(yo)
yo = LeakyReLU()(yo)
yo = BatchNormalization()(yo)
yo = GlobalMaxPooling2D()(yo)
yo = Dropout(0.1)(yo)
...
model.fit_generator(
        #  train_x, train_y,
        train_gen.flow(train_x, train_y, batch_size=49),
        steps_per_epoch=len(train_x)//49,
        #  batch_size = 30,
        epochs=300,
        validation_data=(valid_x, valid_y),
        callbacks=[
            ModelCheckpoint('685.h5', monitor='val_acc', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1),
            history
        ])

#  hisfile = open("history", "w")
#  hist = {
